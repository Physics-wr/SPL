...
class EmbeddingHead(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.ParameterList([nn.Parameter(torch.Tensor(i, feat_dim)) for i in num_pids])
        self.cls_layer = nn.ModuleList([getattr(any_softmax, cls_type)(i, scale, margin) for i in num_pids])
    ...
    def from_config(cls, cfg):
        ...
        num_pids      = cfg.DATASETS.NUM_PIDS
        # fmt: on
        return {
            'feat_dim': feat_dim,
    ...
    def forward(self, features, data_name:int=None, targets=None):
        """
        ...
        # Training
        if self.cls_layer[0].__class__.__name__ == 'Linear':
            logits = F.linear(neck_feat, self.weight[data_name])
        else:
            logits = F.linear(F.normalize(neck_feat), F.normalize(self.weight[data_name]))
        
        # Pass logits.clone() into cls_layer, because there is in-place operations
        cls_outputs = self.cls_layer[data_name](logits.clone(), targets)
        ...
        return {
            ...
            "pred_class_logits": logits.mul(self.cls_layer[data_name].s),
            "features": feat,
        }
